        ...
        <div class="container mx-auto flex justify-between items-center">
            ...
            <ul class="flex space-x-6 items-center">
                ...
                <li class="relative list-none">
                    ...
                    <button class="relative px-4 py-2 rounded-md font-semibold"
                ...
                <li x-data="{ open: false }" class="relative">
                    <button @click="open = !open"
                        class="flex items-center space-x-2 px-4 py-2 rounded-md font-semibold">
                        <img src="{{ Auth::user()->photo ? asset('storage/' . Auth::user()->photo) : '' }}" alt="Avatar"
                            class="w-8 h-8 rounded-full">
                        <span>{{ Auth::user()->name }}</span>
                    </button>
                    <div x-show="open" @click.away="open = false"
                        class="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-lg p-3">
                        <a href="{{ route('profile.edit') }}" class="block px-3 py-2 text-sm">Edit Profil</a>
                        <a href="{{ route('customer.pesanan.index') }}" class="block px-3 py-2 text-sm">Pesanan</a>
                        <a href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                            class="block px-3 py-2 text-sm text-red-500">Logout</a>
                ...
                <li>
                    <a href="{{ route('login') }}"
                        class="bg-[#E81500] text-white px-4 py-2 rounded-md font-semibold">Masuk</a>
                </li>
                @endauth
            </ul>
        </div>
