        ...
        <div class="flex-1 flex flex-col">
                    ...
                    <div class="relative">
                        <button @click="showNotif = !showNotif" class="relative">
                            ...
                            @if(isset($notifications) && count($notifications) > 0)
                                <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">{{ count($notifications) }}</span>
                            @endif
                        </button>
                        <div x-show="showNotif" @click.away="showNotif = false" class="absolute right-0 mt-2 w-64 bg-white shadow-lg rounded-lg p-4">
            ...
            <main class="p-6">
                ...
                <div class="grid grid-cols-4 gap-4 mb-6">
                    <a href="{{ route('admin.orders.all') }}" class="bg-white p-5 rounded-lg shadow text-center block hover:bg-gray-100">
                        <h3 class="text-lg font-semibold text-gray-700">Total Pesanan</h3>
                        <p class="text-2xl font-bold text-gray-800">{{ $total }}</p>
                    </a>
                    <a href="{{ route('admin.orders.status', 'completed') }}" class="bg-white p-5 rounded-lg shadow text-center block hover:bg-gray-100">
                        <h3 class="text-lg font-semibold text-gray-700">Selesai</h3>
                        <p class="text-2xl font-bold text-gray-800">{{ $completed }}</p>
                    </a>
                    <a href="{{ route('admin.orders.status', 'pending') }}" class="bg-white p-5 rounded-lg shadow text-center block hover:bg-gray-100">
                        <h3 class="text-lg font-semibold text-gray-700">Menunggu Konfirmasi</h3>
                        <p class="text-2xl font-bold text-gray-800">{{ $pending }}</p>
                    </a>
                    <a href="{{ route('admin.orders.status', 'rejected') }}" class="bg-white p-5 rounded-lg shadow text-center block hover:bg-gray-100">
